...
class Actor(nn.Module):
    ...
    def __init__(self, state_size, action_size, seed, hidden_size=32, init_w=3e-3, log_std_min=-20, log_std_max=2):
        ...
        # As we can see, it provides a mean value and a standard.


    def reset_parameters(self):
        self.fc1.weight.data.uniform_(*hidden_init(self.fc1))
        self.fc2.weight.data.uniform_(*hidden_init(self.fc2))
        self.mu.weight.data.uniform_(-self.init_weight , self.init_weight )
        self.log_std_linear.weight.data.uniform_(-self.init_weight , self.init_weight )
        # 🐶weight.data.uniform_ is a method in PyTorch, which is a popular deep learning framework. 
        # This method is used to initialize the weights of a tensor with values drawn from a uniform distribution.
        #In PyTorch, tensors are multi-dimensional arrays that can represent parameters or data in neural networks. The uniform_ function initializes the tensor's elements with random numbers generated from a uniform distribution.
    ...
    def get_action(self, state):
        ...
        std = log_std.exp()
        dist = Normal(0, 1)
        e      = dist.sample().to(device) # THis is a sampling, we can find somewhere else.
...
class ReplayBuffer:
    ...
    def __init__(self, action_size, buffer_size, batch_size, seed):
        ...
        # print('seed in ReplayBuffer:', self.seed )
